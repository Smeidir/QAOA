...
def parallell_runner(parameters, graph,name):
    ...
    qaoa_variant=parameters[2], warm_start=parameters[3], errors = parameters[4],depth = parameters[5], recursive_vars = parameters[6])
    qaoa.build_circuit()
    qaoa.run()
    solver = Solver(graph)
    ...
    'errors':parameters[4], 'depth' : parameters[5], 'recursiv_vars':parameters[6], 'graph_size': len(graph.nodes()), 'graph_name' : name,
